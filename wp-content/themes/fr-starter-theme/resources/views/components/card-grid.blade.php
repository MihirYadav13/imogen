<div class="container-fluid card-grid-container ajax-container" ajax-config="{{ $ajaxConfig }}" connected-filters="{{ json_encode($connectedFilters) }}" fr-status="{{ empty($posts)?'no-results-found':(!$hasMore?'no-more-results':'') }}">
